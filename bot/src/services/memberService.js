function createMemberService({ guildRepository, memberRepository, departmentRepository }) {
  ...
  function getMemberWithDepartments(discordGuild, nickname) {
    const guild = guildRepository.ensureGuild(discordGuild.id, discordGuild.name);
    return memberRepository.findByNicknameWithDepartments(guild.id, nickname);
  }

  function listMembersByDepartment(discordGuild, departmentId) {
    const guild = guildRepository.ensureGuild(discordGuild.id, discordGuild.name);
    return memberRepository.listByDepartment(guild.id, departmentId);
  }

  function listAllMembers(discordGuild) {
    const guild = guildRepository.ensureGuild(discordGuild.id, discordGuild.name);
    return memberRepository.findAll(guild.id);
  }

  return { addMember, getMemberInfo, getMemberWithDepartments, listMembersByDepartment, listAllMembers };
}
